...
ROOT_DIR='/c/code/osg_dependencies_'
...
MINGW_INSTALL_DIR='/c/Tools/MinGW'
...
#*****[Libs needed for OSG]*****************************************************************
BUILD_JPEG=false
BUILD_JPEG_TURBO=false
BUILD_PNG=false
BUILD_XZ_LZMA=false
BUILD_TIFF=false
BUILD_FREETYPE=false
BUILD_UNGIF=false
BUILD_FONTCONFIG=false
BUILD_PIXMAN=false
BUILD_CAIRO=false
BUILD_OPENSSL=false
BUILD_CURL=false
...
if type $CMAKE_EXE 2>/dev/null; then
   ...
   echo "CMAKE is not installed. Download the latest version from http://www.cmake.org/download/ and install it and add it to the path."; 
   #exit;     
fi
...
#*******************************[Build JPEG]***********************************
LIBNAME_ICONV_DIR='jpeg'
LIBNAMEVERSION_ICONV_DIR='jpeg-9b'
if $BUILD_JPEG ; then
    #Change to downloads folder
    cd $DOWNLOAD_DIR

    SOURCE_ARCHIVE='jpegsr9b.zip'
    SOURCE_URL='http://www.ijg.org/files/jpegsr9b.zip'
    SOURCE_EXTRACT_DIR=$LIBSEXTERNAL_DIR/Gis/$LIBNAME_ICONV_DIR

    # Getting and building JPEG
    echo && echo -e $GREEN  Getting and building $LIBNAMEVERSION_ICONV_DIR $NORMAL
    #pwd
    echo -e $GREEN  removing previous stuff $NORMAL
    #remove the previous extracted files of JPEG
    rm -rf $SOURCE_EXTRACT_DIR
    ...
    cd $SOURCE_EXTRACT_DIR/$LIBNAMEVERSION_ICONV_DIR
    #pwd
    echo -e $GREEN  configure JPEG makefiles $NORMAL
	autoheader
    echo -e $YELLOW && ./configure && echo -e $NORMAL
    echo -e $GREEN  ---------------- Build Binaries -------------------- $NORMAL
    $MAKETOOL && $MAKETOOL install-strip
...
#*******************************[Build JPEG_TURBO]***********************************
# The NASM assembler is absolutely required: if you don't have it already installed on your system, you can download and install now.
# http://www.nasm.us/pub/nasm/releasebuilds/2.12.02/win32/nasm-2.12.02-win32.zip 
# and inzip it in C:\Tools
#
LIBNAME_ICONV_DIR='jpeg'
LIBNAMEVERSION_ICONV_DIR='libjpeg-turbo-1.4.2'
if $BUILD_JPEG_TURBO ; then
    #Change to downloads folder
    cd $DOWNLOAD_DIR

    SOURCE_ARCHIVE='libjpeg-turbo-1.4.2.tar.gz'
    SOURCE_URL='http://sourceforge.net/projects/libjpeg-turbo/files/1.4.2/libjpeg-turbo-1.4.2.tar.gz'
    SOURCE_EXTRACT_DIR=$LIBSEXTERNAL_DIR/Gis/$LIBNAME_ICONV_DIR

    # Getting and building JPEG_TURBO
    echo && echo -e $GREEN  Getting and building $LIBNAMEVERSION_ICONV_DIR $NORMAL
    #pwd
    echo -e $GREEN  removing previous stuff $NORMAL
    #remove the previous extracted files of JPEG_TURBO
    rm -rf $SOURCE_EXTRACT_DIR
    ...
    cd $SOURCE_EXTRACT_DIR/$LIBNAMEVERSION_ICONV_DIR
    pwd
    echo -e $GREEN  configure JPEG_TURBO makefiles $NORMAL
	export PATH=/c/Tools/nasm-2.12.02:$PATH
	echo $PATH	
    echo -e $YELLOW && ./configure --prefix=/usr/local && echo -e $NORMAL
    echo -e $GREEN  ---------------- Build Binaries -------------------- $NORMAL
    $MAKETOOL || exit 1 && $MAKETOOL install-strip
    echo
fi
#*******************************[Build JPEG_TURBO]***********************************
...
#*******************************[Build PNG]***********************************
LIBNAME_ICONV_DIR='libpng'
LIBNAMEVERSION_ICONV_DIR='libpng-1.6.23'
if $BUILD_PNG ; then
    #Change to downloads folder
    cd $DOWNLOAD_DIR

    SOURCE_ARCHIVE='libpng-1.6.23.tar.gz'
    SOURCE_URL='http://downloads.sourceforge.net/project/libpng/libpng16/1.6.23/libpng-1.6.23.tar.gz'
    SOURCE_EXTRACT_DIR=$LIBSEXTERNAL_DIR/Gis/$LIBNAME_ICONV_DIR

    # Getting and building PNG
    echo && echo -e $GREEN  Getting and building $LIBNAMEVERSION_ICONV_DIR $NORMAL
    #pwd
    echo -e $GREEN  removing previous stuff $NORMAL
    #remove the previous extracted files of PNG
    rm -rf $SOURCE_EXTRACT_DIR
    ...
    cd $SOURCE_EXTRACT_DIR/$LIBNAMEVERSION_ICONV_DIR
    #pwd
    echo -e $GREEN  configure PNG makefiles $NORMAL
...
#*******************************[Build XZ_LZMA]***********************************
LIBNAME_ICONV_DIR='xz_lzma'
LIBNAMEVERSION_ICONV_DIR='xz-5.2.2'
if $BUILD_XZ_LZMA ; then
    #Change to downloads folder
    cd $DOWNLOAD_DIR

    SOURCE_ARCHIVE='xz-5.2.2.tar.gz'
    SOURCE_URL='http://tukaani.org/xz/xz-5.2.2.tar.gz'
    SOURCE_EXTRACT_DIR=$LIBSEXTERNAL_DIR/Gis/$LIBNAME_ICONV_DIR

    # Getting and building XZ_LZMA
    echo && echo -e $GREEN  Getting and building $LIBNAMEVERSION_ICONV_DIR $NORMAL
    #pwd
    echo -e $GREEN  removing previous stuff $NORMAL
    #remove the previous extracted files of XZ_LZMA
    rm -rf $SOURCE_EXTRACT_DIR
    ...
    cd $SOURCE_EXTRACT_DIR/$LIBNAMEVERSION_ICONV_DIR
    #pwd
    echo -e $GREEN  configure XZ_LZMA makefiles $NORMAL
    echo -e $YELLOW && ./configure && echo -e $NORMAL
    echo -e $GREEN  ---------------- Build Binaries -------------------- $NORMAL
    $MAKETOOL && $MAKETOOL install-strip
    echo
...
#*******************************[Build TIFF]***********************************
LIBNAME_ICONV_DIR='tiff'
LIBNAMEVERSION_ICONV_DIR='tiff-4.0.6'
if $BUILD_TIFF ; then
    #Change to downloads folder
    cd $DOWNLOAD_DIR

    SOURCE_ARCHIVE='tiff-4.0.6.zip'
    SOURCE_URL='http://download.osgeo.org/libtiff/tiff-4.0.6.zip'
    SOURCE_EXTRACT_DIR=$LIBSEXTERNAL_DIR/Gis/$LIBNAME_ICONV_DIR

    # Getting and building TIFF
    echo && echo -e $GREEN  Getting and building $LIBNAMEVERSION_ICONV_DIR $NORMAL
    #pwd
    echo -e $GREEN  removing previous stuff $NORMAL
    #remove the previous extracted files of TIFF
    rm -rf $SOURCE_EXTRACT_DIR
    ...
    cd $SOURCE_EXTRACT_DIR/$LIBNAMEVERSION_ICONV_DIR
    #pwd
    echo -e $GREEN  configure TIFF makefiles $NORMAL
    echo -e $YELLOW && ./configure && echo -e $NORMAL
    echo -e $GREEN  ---------------- Build Binaries -------------------- $NORMAL
    $MAKETOOL && $MAKETOOL install-strip
    echo
...
#*******************************[Build FREETYPE]***********************************
LIBNAME_ICONV_DIR='freetype'
LIBNAMEVERSION_ICONV_DIR='freetype-2.6'
if $BUILD_FREETYPE ; then
    #Change to downloads folder
    cd $DOWNLOAD_DIR

    SOURCE_ARCHIVE='freetype-2.6.tar.bz2'
    SOURCE_URL='http://download.savannah.gnu.org/releases/freetype/freetype-2.6.tar.bz2'
    SOURCE_EXTRACT_DIR=$LIBSEXTERNAL_DIR/Gis/$LIBNAME_ICONV_DIR

    # Getting and building FREETYPE
    echo && echo -e $GREEN  Getting and building $LIBNAMEVERSION_ICONV_DIR $NORMAL
    #pwd
    echo -e $GREEN  removing previous stuff $NORMAL
    #remove the previous extracted files of FREETYPE
    rm -rf $SOURCE_EXTRACT_DIR
    ...
    cd $SOURCE_EXTRACT_DIR/$LIBNAMEVERSION_ICONV_DIR
    #pwd
    echo -e $GREEN  configure FREETYPE makefiles $NORMAL
    echo -e $YELLOW && ./configure && echo -e $NORMAL
    echo -e $GREEN  ---------------- Build Binaries -------------------- $NORMAL
    $MAKETOOL && $MAKETOOL install-strip
    echo
...
#*******************************[Build UNGIF]***********************************
LIBNAME_ICONV_DIR='libungif'
LIBNAMEVERSION_ICONV_DIR='libungif-4.1.4'
if $BUILD_UNGIF ; then
    #Change to downloads folder
    cd $DOWNLOAD_DIR

    SOURCE_ARCHIVE='libungif-4.1.4.tar.gz'
    SOURCE_URL='http://downloads.sourceforge.net/project/giflib/libungif-4.x/libungif-4.1.4/libungif-4.1.4.tar.gz'
    SOURCE_EXTRACT_DIR=$LIBSEXTERNAL_DIR/Gis/$LIBNAME_ICONV_DIR

    # Getting and building UNGIF
    echo && echo -e $GREEN  Getting and building $LIBNAMEVERSION_ICONV_DIR $NORMAL
    #pwd
    echo -e $GREEN  removing previous stuff $NORMAL
    #remove the previous extracted files of UNGIF
    rm -rf $SOURCE_EXTRACT_DIR
    ...
    cd $SOURCE_EXTRACT_DIR/$LIBNAMEVERSION_ICONV_DIR
    #pwd
    echo -e $GREEN  configure UNGIF makefiles $NORMAL
    echo -e $YELLOW && ./configure && echo -e $NORMAL
	cd util
	echo -e $RED Fix the makefile $NORMAL
	sed -i 's/)  gifclrmp gifrotat/)  gifclrmp$(EXEEXT) gifrotat$(EXEEXT)/' Makefile
	cd ../
    echo -e $GREEN  ---------------- Build Binaries -------------------- $NORMAL
    $MAKETOOL && $MAKETOOL install-strip
    echo
...
#*******************************[Build FONTCONFIG]***********************************
LIBNAME_ICONV_DIR='fontconfig'
LIBNAMEVERSION_ICONV_DIR='fontconfig-2.12.0'
if $BUILD_FONTCONFIG ; then
    #Change to downloads folder
    cd $DOWNLOAD_DIR

    SOURCE_ARCHIVE='fontconfig-2.12.0.tar.gz'
    SOURCE_URL='https://www.freedesktop.org/software/fontconfig/release/fontconfig-2.12.0.tar.gz'
    SOURCE_EXTRACT_DIR=$LIBSEXTERNAL_DIR/Gis/$LIBNAME_ICONV_DIR

    # Getting and building FONTCONFIG
    echo && echo -e $GREEN  Getting and building $LIBNAMEVERSION_ICONV_DIR $NORMAL
    #pwd
    echo -e $GREEN  removing previous stuff $NORMAL
    #remove the previous extracted files of FONTCONFIG
    rm -rf $SOURCE_EXTRACT_DIR
    ...
    cd $SOURCE_EXTRACT_DIR/$LIBNAMEVERSION_ICONV_DIR
    #pwd
    echo -e $GREEN  configure FONTCONFIG makefiles $NORMAL
	export 'CFLAGS=-I/usr/local/include'
	export 'LDFLAGS=-L/usr/local/lib'
	export 'PKG_CONFIG_PATH=/usr/local/lib/pkgconfig'
    echo -e $YELLOW && ./configure --disable-docs --enable-shared && echo -e $NORMAL
    echo -e $GREEN  ---------------- Build Binaries -------------------- $NORMAL
    $MAKETOOL && $MAKETOOL install-strip
    echo
fi
...
#*******************************[Build PIXMAN]***********************************
LIBNAME_ICONV_DIR='pixman'
LIBNAMEVERSION_ICONV_DIR='pixman-0.34.0'
if $BUILD_PIXMAN ; then
    #Change to downloads folder
    cd $DOWNLOAD_DIR

    SOURCE_ARCHIVE='pixman-0.34.0.tar.gz'
    SOURCE_URL='http://cairographics.org/releases/pixman-0.34.0.tar.gz'
    SOURCE_EXTRACT_DIR=$LIBSEXTERNAL_DIR/Gis/$LIBNAME_ICONV_DIR

    # Getting and building PIXMAN
    echo && echo -e $GREEN  Getting and building $LIBNAMEVERSION_ICONV_DIR $NORMAL
    #pwd
    echo -e $GREEN  removing previous stuff $NORMAL
    #remove the previous extracted files of PIXMAN
    rm -rf $SOURCE_EXTRACT_DIR
    ...
    cd $SOURCE_EXTRACT_DIR/$LIBNAMEVERSION_ICONV_DIR
    #pwd
    echo -e $GREEN  configure PIXMAN makefiles $NORMAL
	export "CFLAGS=-I/usr/local/include"
	export "LDFLAGS=-L/usr/local/lib"
	export "PKG_CONFIG_PATH=/usr/local/lib/pkgconfig"
    echo -e $YELLOW && ./configure && echo -e $NORMAL
...
#*******************************[Build CAIRO]***********************************
LIBNAME_ICONV_DIR='cairo'
LIBNAMEVERSION_ICONV_DIR='cairo-1.14.6'
if $BUILD_CAIRO ; then
    #Change to downloads folder
    cd $DOWNLOAD_DIR

    SOURCE_ARCHIVE='cairo-1.14.6.tar.xz'
    SOURCE_URL='http://cairographics.org/releases/cairo-1.14.6.tar.xz'
    SOURCE_EXTRACT_DIR=$LIBSEXTERNAL_DIR/Gis/$LIBNAME_ICONV_DIR

    # Getting and building CAIRO
    echo && echo -e $GREEN  Getting and building $LIBNAMEVERSION_ICONV_DIR $NORMAL
    #pwd
    echo -e $GREEN  removing previous stuff $NORMAL
    #remove the previous extracted files of CAIRO
    rm -rf $SOURCE_EXTRACT_DIR
    ...
    #Extract the zip file
    echo -e $RED Extract the source $NORMAL
    tar -xf $SOURCE_ARCHIVE -C $SOURCE_EXTRACT_DIR
	#$ZIPTOOL $SOURCE_ARCHIVE -o$SOURCE_EXTRACT_DIR	
	# cd to extracted directory
    cd $SOURCE_EXTRACT_DIR/$LIBNAMEVERSION_ICONV_DIR
    #pwd
    echo -e $GREEN  configure CAIRO makefiles $NORMAL
	export 'PKG_CONFIG_PATH=/usr/local/lib/pkgconfig'
	export "CFLAGS=-I/usr/local/include"
	export "LDFLAGS=-L/usr/local/lib"
    echo -e $YELLOW && ./configure --disable-pthread && echo -e $NORMAL
    echo -e $GREEN  ---------------- Build Binaries -------------------- $NORMAL
    $MAKETOOL && $MAKETOOL install-strip
    echo
fi
...
#*******************************[Build OPENSSL]***********************************
LIBNAME_ICONV_DIR='openssl'
LIBNAMEVERSION_ICONV_DIR='openssl-1.0.2h'
if $BUILD_OPENSSL ; then
    #Change to downloads folder
    cd $DOWNLOAD_DIR

    SOURCE_ARCHIVE='openssl-1.0.2h.tar.gz'
    SOURCE_URL='http://www.openssl.org/source/openssl-1.0.2h.tar.gz'
    SOURCE_EXTRACT_DIR=$LIBSEXTERNAL_DIR/Gis/$LIBNAME_ICONV_DIR

    # Getting and building OPENSSL
    echo && echo -e $GREEN  Getting and building $LIBNAMEVERSION_ICONV_DIR $NORMAL
    #pwd
    echo -e $GREEN  removing previous stuff $NORMAL
    #remove the previous extracted files of OPENSSL
    rm -rf $SOURCE_EXTRACT_DIR

    #Getting the source from the internet
    [ ! -f $SOURCE_ARCHIVE ] && echo -e $CYAN && wget --secure-protocol=auto $SOURCE_URL && echo -e $NORMAL
    ...
    cd $SOURCE_EXTRACT_DIR/$LIBNAMEVERSION_ICONV_DIR
    #pwd
    echo -e $GREEN  configure OPENSSL makefiles $NORMAL
	export "CFLAGS=-I/usr/local/include"
	export "LDFLAGS=-L/usr/local/lib"
	export "PKG_CONFIG_PATH=/usr/local/lib/pkgconfig"
    echo -e $YELLOW && ./configure mingw --prefix=/usr/local shared && echo -e $NORMAL
    echo -e $GREEN  ---------------- Build Binaries -------------------- $NORMAL
    $MAKETOOL depend && $MAKETOOL && $MAKETOOL install-strip
    echo
fi
#*******************************[Build OPENSSL]***********************************

#*******************************[Build CURL]***********************************
LIBNAME_ICONV_DIR='curl'
LIBNAMEVERSION_ICONV_DIR='curl-7.49.1'
if $BUILD_CURL ; then
    #Change to downloads folder
    cd $DOWNLOAD_DIR

    SOURCE_ARCHIVE='curl-7.49.1.zip'
    SOURCE_URL='https://curl.haxx.se/download/curl-7.49.1.zip'
    SOURCE_EXTRACT_DIR=$LIBSEXTERNAL_DIR/Gis/$LIBNAME_ICONV_DIR

    # Getting and building CURL
    echo && echo -e $GREEN  Getting and building $LIBNAMEVERSION_ICONV_DIR $NORMAL
    #pwd
    echo -e $GREEN  removing previous stuff $NORMAL
    #remove the previous extracted files of CURL
    rm -rf $SOURCE_EXTRACT_DIR
    ...
    cd $SOURCE_EXTRACT_DIR/$LIBNAMEVERSION_ICONV_DIR
    #pwd
    echo -e $GREEN  configure CURL makefiles $NORMAL
	export 'PKG_CONFIG_PATH=/usr/local/lib/pkgconfig'
	export "CFLAGS=-I/usr/local/include"
	export "LDFLAGS=-L/usr/local/lib"
    echo -e $YELLOW && ./configure --enable-shared=yes --with-zlib=/usr/local && echo -e $NORMAL
    echo -e $GREEN  ---------------- Build Binaries -------------------- $NORMAL
    $MAKETOOL && $MAKETOOL install-strip
    echo
fi
...
#*******************************[Build MINIZIP]***********************************
LIBNAME_ICONV_DIR='minizip'
LIBNAMEVERSION_ICONV_DIR='minizip-1.2.8'
if $BUILD_MINIZIP ; then
    #Change to downloads folder
    cd $DOWNLOAD_DIR

    SOURCE_ARCHIVE='minizip-1.2.8.tar.gz'
    SOURCE_URL='http://www.gaia-gis.it/gaia-sins/dataseltzer-sources/minizip-1.2.8.tar.gz'
    SOURCE_EXTRACT_DIR=$LIBSEXTERNAL_DIR/Gis/$LIBNAME_ICONV_DIR

    # Getting and building MINIZIP
    echo && echo -e $GREEN  Getting and building $LIBNAMEVERSION_ICONV_DIR $NORMAL
    #pwd
    echo -e $GREEN  removing previous stuff $NORMAL
    #remove the previous extracted files of MINIZIP
    rm -rf $SOURCE_EXTRACT_DIR
    ...
    cd $SOURCE_EXTRACT_DIR/$LIBNAMEVERSION_ICONV_DIR
    #pwd
    echo -e $GREEN  configure MINIZIP makefiles $NORMAL
	export "CFLAGS=-I/usr/local/include"
	export "LDFLAGS=-L/usr/local/lib"
	export "PKG_CONFIG_PATH=/usr/local/lib/pkgconfig"
    echo -e $YELLOW && ./configure --enable-shared=yes && echo -e $NORMAL
    echo -e $GREEN  ---------------- Build Binaries -------------------- $NORMAL
    $MAKETOOL && $MAKETOOL install-strip
    echo
fi
...
#*******************************[Build PCRE]***********************************
LIBNAME_ICONV_DIR='pcre'
LIBNAMEVERSION_ICONV_DIR='pcre-8.39'
if $BUILD_PCRE ; then
    #Change to downloads folder
    cd $DOWNLOAD_DIR

    SOURCE_ARCHIVE='pcre-8.39.tar.gz'
    SOURCE_URL='http://downloads.sourceforge.net/project/pcre/pcre/8.39/pcre-8.39.tar.gz'
    SOURCE_EXTRACT_DIR=$LIBSEXTERNAL_DIR/Gis/$LIBNAME_ICONV_DIR

    # Getting and building PCRE
    echo && echo -e $GREEN  Getting and building $LIBNAMEVERSION_ICONV_DIR $NORMAL
    #pwd
    echo -e $GREEN  removing previous stuff $NORMAL
    #remove the previous extracted files of PCRE
    rm -rf $SOURCE_EXTRACT_DIR
    ...
    cd $SOURCE_EXTRACT_DIR/$LIBNAMEVERSION_ICONV_DIR
    #pwd
    echo -e $GREEN  configure PCRE makefiles $NORMAL
	export "CFLAGS=-I/usr/local/include"
	export "LDFLAGS=-L/usr/local/lib"
	export "PKG_CONFIG_PATH=/usr/local/lib/pkgconfig"
    echo -e $YELLOW && ./configure && echo -e $NORMAL
...
#*******************************[Build COLLADA_DOM]***********************************
LIBNAME_ICONV_DIR='collada-dom'
LIBNAMEVERSION_ICONV_DIR='collada-dom-2.4.0'
if $BUILD_COLLADA_DOM ; then
    #Change to downloads folder
    cd $DOWNLOAD_DIR

    SOURCE_ARCHIVE='collada-dom-2.4.0.tgz'
    SOURCE_URL='http://downloads.sourceforge.net/project/collada-dom/Collada%20DOM/Collada%20DOM%202.4/collada-dom-2.4.0.tgz'
    SOURCE_EXTRACT_DIR=$LIBSEXTERNAL_DIR/Gis/$LIBNAME_ICONV_DIR

    # Getting and building COLLADA_DOM
    echo && echo -e $GREEN  Getting and building $LIBNAMEVERSION_ICONV_DIR $NORMAL
    #pwd
    echo -e $GREEN  removing previous stuff $NORMAL
    #remove the previous extracted files of COLLADA_DOM
    rm -rf $SOURCE_EXTRACT_DIR
    ...
    #pwd
	exit;
    echo -e $GREEN  configure COLLADA_DOM makefiles $NORMAL
	export "BOOST_ROOT=/c/code/osg_dependencies_/LibsDep_/boost_1_61_0"
	rm -rf build
    mkdir -p build
    cd build
...
LIB_GEOS_NAMEVERSION_DIR='geos-3.5.0'
if $BUILD_GEOS ; then
    #Change to downloads folder
    cd $DOWNLOAD_DIR

    SOURCE_ARCHIVE='geos-3.5.0.tar.bz2'
...
LIB_FREEXL_NAMEVERSION_DIR=$LIB_FREEXL_NAME_DIR'-1.0.2'
...
LIB_PG_NAMEVERSION_DIR='postgresql-9.5.3'
if $BUILD_POSTGRES ; then
    ...
    BINARY_ARCHIVE='postgresql-9.5.3-1-windows-binaries.zip'
    SOURCE_URL='http://get.enterprisedb.com/postgresql/postgresql-9.5.3-1-windows-binaries.zip'
...
LIB_SQLLITE_VERSION='3.13.0.0'
LIB_SQLLITE_NAMEVERSION_DIR=$LIB_SQLLITE_NAME_DIR'-'$LIB_SQLLITE_VERSION
if $BUILD_SQLite ; then
    #Change to downloads folder
    cd $DOWNLOAD_DIR

    SOURCE_ARCHIVE='sqlite-autoconf-3130000.tar.gz'
    SOURCE_URL='https://www.sqlite.org/2016/'$SOURCE_ARCHIVE
    ...
    echo -e $YELLOW && ./configure &&  echo -e $NORMAL
    echo -e $GREEN  ---------------- Build Binaries -------------------- $NORMAL
    $MAKETOOL || exit 1 && $MAKETOOL install
    echo
fi
#*******************************[Build SQLite]**********************************
...
if $BUILD_SPATIALITE ; then
    ...
    LIBNAMEVERSION_DIR='libspatialite-4.4.0-RC1'
    SOURCE_ARCHIVE='libspatialite-4.4.0-RC1.tar.gz'
    SOURCE_URL='http://www.gaia-gis.it/gaia-sins/libspatialite-sources/libspatialite-4.4.0-RC1.tar.gz'
    SOURCE_EXTRACT_DIR=$LIBSEXTERNAL_DIR/Gis/$LIBNAME_DIR
	...
	export 'PKG_CONFIG=/usr/bin/pkg-config.exe'
    ...
    echo -e $YELLOW && ./configure --target=mingw32 --enable-gcp=yes #--enable-lwgeom=yes 
    echo -e $NORMAL
    make && make install-strip
...
if $BUILD_GDAL ; then
    ...
    LIBNAME_DIR='gdal'
    LIBNAMEVERSION_DIR='gdal-1.11.5'
    SOURCE_ARCHIVE='gdal1115.zip'
    SOURCE_URL='http://download.osgeo.org/gdal/1.11.5/gdal1115.zip'
    SOURCE_EXTRACT_DIR=$LIBSEXTERNAL_DIR/Gis/$LIBNAME_DIR
    ...
    #pwd
    echo -e $GREEN  configure makefiles $NORMAL
	echo -e $GREEN $LIBSDEP_DIR/$LIBNAME_PG_DIR/$LIB_PG_NAMEVERSION_DIR/win32/pgsql/bin/pg_config.exe $NORMAL
	#export PATH=$LIBSDEP_DIR/$LIBNAME_PG_DIR/$LIB_PG_NAMEVERSION_DIR/win32/pgsql/bin:$PATH
	echo $PATH
